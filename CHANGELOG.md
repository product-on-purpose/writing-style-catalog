...
- **Stale "Often confused with" sections on baseline entries**, surfaced by a whole-corpus de-duplication audit (all 72 formats reviewed by family cluster, not just against each entry's one declared neighbor). Three stable entries had a body disambiguation that contradicted their own `confusable_with` frontmatter (leftover seed text): `blog-post-long-form` now contrasts `whitepaper` instead of adr/prd, `slack-message` now contrasts `email` instead of prd/adr, and `devotional-entry` now contrasts `sermon` (replacing a note whose only overlap with a PRD was the shared word "entry"). `technical-reference` had its `confusable_with` reconciled with the neighbors its body actually discusses. The audit also sharpened two close opinion formats so their output diverges on substance rather than byline (`op-ed` draws on the named author's personal stake; `editorial` on collective institutional judgment) and added five missing reciprocal `confusable_with` links. No entry was promoted and the catalog stayed schema-valid throughout. Details in `docs/internal/release-plans/stream-b-breadth-status.md`.
