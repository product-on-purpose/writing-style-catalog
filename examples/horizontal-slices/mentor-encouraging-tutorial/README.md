...
A composition of `friendly-mentor` voice, `encouraging` tone, `procedural` style, and `readme` format. Used for onboarding documentation aimed at early-career engineers who are doing each thing for the first time.
...
| Style | `procedural` | The reader arrived to complete a specific task. Prerequisites up front, one action per numbered step, a clear success signal at the end. The structure protects the reader from getting lost in their own first attempt. |
