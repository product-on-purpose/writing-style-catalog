...
The site generator (`scripts/generate_site_pages.py`) reads this tree and renders it into `docs/examples/`, `docs/recipes/`, and the diff-pair pages under `docs/`. Every example validates against `schemas/example.schema.json` (or `schemas/diff-pair.schema.json`) via `python tools/validate.py`. See `REPOSITORY.md` for the full picture.
