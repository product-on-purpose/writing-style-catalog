...
- **Judge is cross-model, not cross-vendor.** Sonnet judging Opus is not the C2 standard (a
  different vendor, e.g. GPT / Codex). A same-vendor judge can still share blind spots. The single
  highest-value follow-up is to re-run the identical blind packet through a cross-vendor judge and
  confirm the 8/8.
...
1. **Cross-vendor confirmation (highest value):** re-run this exact blind packet through a GPT /
   Codex judge to satisfy C2 and retire the same-vendor caveat. Raw renders and the key are in
   `_agent-context/gate-pilot/` (local) and the scored table above.
