...
Schema-checked, deterministic, no judge needed. Every required field is present and substantive: the universal schema's `pairs_well_with`, `avoid_with`, `confusable_with`, `when_to_use`, `when_not_to_use`, `llm_instruction_phrasing` (all already required and ID-validated by `tools/validate.py`), plus the pedagogical fields ADR 0009 ratifies. ADR 0009 was **Accepted 2026-06-20 as a gate-critical subset**: the fields Gate 2 enforces are `tells`, `anti_patterns`, and `failure_modes`. Those three are the **pedagogical-field subset**, and that subset is fully enforced today: required in the universal schema (presence, count band, shape) and substance-checked by `check_pedagogical_bar` in `tools/validate.py` since 2026-06-22, after the 60 were backfilled. Gate 2 has a second half, the **minimum sample count** (12 per entry across the anchor set), which is NOT a static-frontmatter check: an entry carries no samples, so the count can only be verified at gate-run time once the generator renders the entry across the anchor topics. That half therefore rides the model-calling gate build and is not yet enforced; the deterministic 2026-06-22 validator covers the pedagogical-field subset only. The heavier `before_after_example` and `mini_glossary` are deferred by ADR 0009 (not part of the Gate 2 bar unless a later amendment adopts them). This gate is cheap and runs first, because there is no point paying for judge tokens on a candidate that is structurally incomplete.
...
(required in the schema and substance-checked by Gate 2 since 2026-06-22, after the 60 were
