...
- **Q6 - audit the confusable graph: YES, audit; route test-scaffold edges to the existing diff-pairs, no new field.** Audit all ~35 `confusable_with` edges to keep only genuine near-neighbors. The engineered far-contrast test pairs (for example `slack-message`/`prd`) move to the catalog's existing diff-pair artifacts (`examples/diff-pairs/`, governed by `schemas/diff-pair.schema.json`), which already exist - so this improves on Fable's suggested new `diff_pair_with` field by using the home that is already there: **no schema change, no version bump, no extra ADR.** A `diff_pair_with` back-reference field stays an option only if the audit shows the gate needs entry-level back-refs the diff-pair files do not provide. The audit itself is entry-data work batched into Phase 1.
