...
**Status:** OPEN - 8 of 9 P0 rows decided. A1 (taxonomy cuts) was **RATIFIED 2026-06-19**: adopt the 5-domain / 17-family cut with the Fable Q1-Q12 amendments, the `relational` domain renamed `personal`, and the voice axis folded from 6 families to **5** (`pastoral` becomes a `care` subfamily). (Family count corrected to 17 during Phase 1 codification: the per-domain enumeration 8+4+2+1+2 always summed to 17; "16" was a propagated miscount.) ADR 0010 moved Proposed to Accepted. The remaining entry-level data moves the register raised (slug reconciliation Q9, confusable-graph audit Q6, channel-clone collapse Q3, the three borderline placements Q8) are authorized and batched into the Phase 1 field migration so entries are touched once. The seven earlier P0 calls stand (A6, B1, C1, C2, C3, F2, F3). Only **D1 (anchor topics)** remains open among the P0 rows, still deferred pending example review and the expand/randomize-topics question. P1 and P2 rows still `_pending_` except B2 (decided) and A2 (decided 2026-06-19, hard 12 per Q7).
...
|---|---|---|---|---|
| A1 | Accept the v2 taxonomy cuts | Accept as-is | P0 | **RATIFIED 2026-06-19 - adopt v2 with Q2-Q9 amendments; `relational` renamed `personal`; `pastoral` folded into `care` (5 voice families); ADR 0010 Accepted** |
| A2 | Subfamily 12-member trigger: hard vs soft | Hard cutoff | P1 | **DECIDED 2026-06-19 - hard cutoff at 12 (per A1/Q7)** |
...
| B2 | Conflict handling: warn vs block | Warn + precedence, never block | P1 | **DECIDED - warn + precedence, never block** (2026-06-16; ratified via PR #20 / ADR 0016) |
...
**Update (2026-06-16) - the pilot ran.** Q12 ran on 8 near-neighbor pairs spanning all four axes: **8 of 8 correct** blind attribution (forced binary choice, chance ~0.39 percent), with the three genuinely close seams (`narrative-case-study`/`chronological-narrative`, `warm`/`empathetic`, `confident`/`resolute`) landing at the "subtle" band and the rest clear or dramatic. This is the first direct evidence the gate's bar is achievable on near neighbors, not only the far contrasts the original 8/8 used, which is the precondition the agentic-first bet (F3) depends on. Full method, scored table, and caveats: [`gate-pilot-2026-06-16.md`](gate-pilot-2026-06-16.md). One open follow-up before this fully satisfies C2 - judge policy: the judge was Sonnet 4.6 (a cross-*model* proxy) because Fable was unavailable; a cross-*vendor* judge (GPT/Codex) on the same packet is the recommended confirmation. The pilot does not settle the C1 quality-rubric sub-problem (distinguishability is validated; quality is not).

**Cross-vendor confirmation (2026-06-16).** The identical blind packet was re-judged by Codex (OpenAI GPT): also **8 of 8 correct**, so two independent vendors agree (16 of 16 attributions across Anthropic Sonnet and OpenAI GPT). This retires the same-vendor caveat and satisfies C2 - judge policy's cross-family intent for this pilot. The two judges agreed on exactly one pair being "subtle" - `warm`/`empathetic` - making it the most robust sharpening target. C1's quality rubric remains undefined and gate-blocking. With distinguishability now confirmed cross-vendor on near neighbors, the remaining gates on A1 are working the Q1-Q12 placement/structure items (Q2-Q9), not the gate's basic viability.

**RATIFIED (2026-06-19) - A1 adopted, ADR 0010 Accepted.** The maintainer worked the Fable Q1-Q12 register and ratified the v2 structure. ADR [`0010`](../../adr/0010-domain-and-family-organization.md) moved Proposed to Accepted with the amendments below. The register resolves as follows:

- **Q1 - ratify the v2 structure: ADOPTED.** Adopt the 5-domain / 17-family / voice-family cut with the Q2-Q9 amendments; do not re-cut. (The format-family count is 17, not the "16" the review and earlier docs carried; the enumeration always summed to 17. Corrected during Phase 1 codification.)
- **Q2 - fix the relational domain: RENAME to `personal`.** The domain is renamed `personal` and redefined to cover both its families. Canonical definition (lands in `tools/taxonomy.py` at Phase 1): *"Writing rooted in the writer's own relationships and lived experience: either addressed to a specific person or small group the writer knows (`correspondence`) or drawn from the writer's experience for a wider readership (`essay`). The unifying thread is the personal source of the writing, not the size of the audience."* Near-miss regression sentence (H3): *"A personal essay published to strangers belongs in `personal` because its subject is the writer's own experience; a how-to guide or explainer addressed to a general audience belongs in `public` because its subject is the reader's task, not the writer's life."*
- **Q3 - channel-as-facet: ENFORCE.** Collapse channel-clones (for example `broadcast`'s `substack-post`/`medium-article`/`blog-post-long-form`) to platform-agnostic forms carrying a `channel:` facet; adopt the admission rule "no two siblings may differ only by channel"; document `messaging` (slack/email/chat/dm) as the one deliberate channel-cut exception, because channel constraints genuinely shape those forms. Cleanup batched into Phase 1. **Confirmed 2026-06-19:** the 15 live format entries contain no channel-clones (the clones are inventory-only, not built), so Q3 is now a documented admission rule with no current entry work; it bites when channel-variant forms are first added.
- **Q4 - gate neighbor definition: tree siblings PLUS declared `confusable_with`.** The gate renders a candidate against its tree siblings and against any entry it declares `confusable_with`, regardless of where that entry sits in the tree. Embedding-nearest-neighbors are demoted to a candidate *proposer*, not a gate input.
- **Q5 - neighbor sets for flat tones/styles: WHOLE-AXIS rendering** while each axis stays small (~15-30), with declared `confusable_with` as the priority subset. Recorded trigger: revisit and add structure if either axis is planned past ~50 entries.
- **Q6 - audit the confusable graph: YES, audit; route test-scaffold edges to the existing diff-pairs, no new field.** Audit all ~35 `confusable_with` edges to keep only genuine near-neighbors. The engineered far-contrast test pairs (for example `slack-message`/`prd`) move to the catalog's existing diff-pair artifacts (`examples/diff-pairs/`, governed by `schemas/diff-pair.schema.json`), which already exist - so this improves on Fable's suggested new `diff_pair_with` field by using the home that is already there: **no schema change, no version bump, no extra ADR.** A `diff_pair_with` back-reference field stays an option only if the audit shows the gate needs entry-level back-refs the diff-pair files do not provide. The audit itself is entry-data work batched into Phase 1. **Executed 2026-06-19:** removed 9 bad edges - 5 implausible far-contrast scaffold links (`devotional-entry`/`prd`, `slack-message`/`prd`, `slack-message`/`adr`, `blog-post-long-form`/`adr`, `blog-post-long-form`/`prd`) and 4 cross-axis style-to-format links (`decision-log`/`adr`, `executive-summary`/`one-pager`, `technical-reference`/`diataxis-explanation`, `technical-reference`/`how-to-tutorial`) - and restored the genuine mutual neighbor where a list was left bare (`slack-message`->`email`, `technical-reference`->`readme`, `blog-post-long-form`->`whitepaper`, `decision-log`<->`executive-summary`). Genuine cross-family confusables (for example `one-pager`/`prd`) were kept. No `diff_pair_with` field was needed. Cross-reference validation stays green.
- **Q7 - subfamily split protocol: keep the HARD 12-member trigger** (resolves A2 - subfamily trigger); pre-design subfamily cuts for the known-fat families (`instruction`, `witness`, `devotion`, `deliberation`, `correspondence`, `tribute`, `broadcast`, `expert`, `progress`, `essay`) with a human naming checkpoint per split. Q12's pilot supports 12 as workable.
- **Q8 - placement moves: ACCEPT the five clear moves** (`whitepaper`, `profile-piece`, `letter-to-future-self`, `satirist`, `job-description`), the `oath-of-office` removal, and the `pastoral` member rename; send the three borderlines (`accessibility-audit`, `press-release`, `cover-letter`) to a small near-neighbor gate run during Phase 1. Entry moves batched into Phase 1.
- **Q9 - slug reconciliation: INVENTORY names become canon.** Rename the 60 live entries to the aspirational-inventory names, preserving aliases for the old slugs (H1). Executed alongside the Phase 1 field migration so entries are touched once.
- **Q10 - human-experience scope: adopt H1 (aliases), H3 (definition quality bar / near-miss), H5 (naming checkpoint) now** as governance rules; defer the site-UX items (H2 task index, H4 one-page map, H6 graph-as-guidance) to later site work.
- **Q11 - reconciliation pass: AUTHORIZED as optional cleanup.** A low-priority delta pass (reviewer reads `decisions.md` and the gate spec, reports which recommendations survive contact with decided items). Not blocking.
- **Q12 - gate pilot: DONE.** Ran 2026-06-16, cross-vendor 16/16 (see the pilot updates above).

**Voice families folded 6 to 5, and the grounding recorded.** The proposed sixth voice family `pastoral` is folded into `care` as a subfamily (`care -> pastoral`). This follows from a 2026-06-19 survey of literary and psychological taxonomies (local artifact: `_agent-context/research/voice-taxonomy-grounding_2026-06-19.md`), whose headline is that **no externally-validated framework can ground a discrete multi-persona voice cut**: the validated models (Big Five, HEXACO) are continuous trait models (wrong shape), and the discrete-persona systems (Jung/Mark-Pearson archetypes, DISC, MBTI, Enneagram, Merrill-Reid) are practitioner conventions, not science. So the families are grounded on (1) communicative function (Miller's genre-as-social-action; Searle's speech acts) as the primary standard, (2) the E1 gate's blind distinguishability as the empirical proof, and (3) the brand-archetype clusters as a recognizability cross-reference only. By standard (1), `pastoral` is not a distinct communicative function from `care` (both accompany and form a reader) - it is `care` in a religious register - so it is a subfamily, not a peer family. The full rationale is in ADR 0010, Section 2a. A side finding (NN/g's four tone dimensions are the one cleanly-validated external standard, and they ground the **Tone** axis, not Voice) is logged for a future Tone pass.

**What this unblocks and what defers.** ADR 0010 is Accepted, so the F2 - migration sequencing tighten-to-required step is no longer gated on A1; Phase 1 (taxonomy codification, optional-with-warning) can proceed and then execute the batched entry moves (Q3 collapse, Q6 audit, Q8 placements, Q9 renames) in one pass. D1 - anchor topics is still deferred (independent of A1's structure). No schema change ships in this ratification; the only schema work, if any, is the optional `diff_pair_with` field, which is not being added.
...
**Maintainer decision:** **DECIDED 2026-06-19 - hard cutoff at 12**, enforced by the cross-field validator (`check_taxonomy_membership()`), not a soft alarm. Recorded as part of A1 ratification via Q7. Rationale: determinism beats per-split judgment under agentic-first authoring; a family quietly ballooning to 30 flat members is exactly the noise the taxonomy exists to prevent, and the forced split is itself gate-checkable. Q12's pilot supports 12 as a workable value; revisit only on gate evidence. Rejected: a soft alarm with per-split maintainer judgment (option B), because it stalls the agentic pipeline whenever the maintainer is unavailable.
...
**Maintainer decision:** **DECIDED - warn + precedence, never block.** Recorded 2026-06-16 (jprisant), ratified by merging the S1 implementation (PR #20) and ADR [`0016`](../../adr/0016-conflict-aware-composition.md). A conflicting selection (a pair where either entry lists the other in `avoid_with`, per B1's symmetric rule) is flagged with a warning but still composes the full instruction, applying voice -> tone -> style -> format precedence so the higher-precedence axis leads. Rationale: `avoid_with` means discouraged, not forbidden; a user may want a deliberate tension, and hard-blocking would break legitimate experimentation. The warning prints to stderr so stdout stays a clean, pipeable prompt. Rejected: hard-block (option B). ADR 0016 moves Proposed -> Accepted with this decision.
...
**Update (2026-06-17) - design drafted (not yet calibrated).** The how-to-measure sub-problem now has a concrete answer, parked for build time. The quality rubric collapses to a single judge question: is the render the genuine register or a caricature, judged against the entry's OWN `failure_modes` / `anti_patterns` (the ADR 0009 / Gate 2 fields), asked in the same cross-family judge pass as distinguishability. It is non-circular (restraint checks the instruction's failure mode, the opposite direction from fidelity, so caricature fails it precisely by over-hitting fidelity) and non-gameable (the bound is the entry's published failure mode, not the judge's taste). The pass bar is a calibration output, not a decision: score the frozen 60 (C3 - held-out reference set) and set the anchor at "as good as the 60"; a baseline that fails means the bar is miscalibrated, not that the baseline must improve. A fuller multi-criterion rubric is deliberately NOT built on speculation; it splits out of the single question only if a calibration run shows one question is too coarse. Full reasoning in [`adherence-gate-spec.md`](adherence-gate-spec.md) ("The restraint check (resolving C1)"). Status: still gate-blocking for E1 (Phase 5 / v0.4.0+), but the rubric is no longer undefined - it is designed, to be calibrated when the gate is built, not before.
...
- **8 of 9 P0 (execution-blocking) rows are decided:** A1 - taxonomy cuts (ratified 2026-06-19), A6 - freeze axes, B1 - avoid_with symmetry, C1 - what the gate scores, C2 - judge policy, C3 - held-out reference set, F2 - migration sequencing, F3 - resourcing posture. ADR 0010 is now Accepted, ratifying both the axis freeze (section 1) and the full domain/family/voice-family structure, so Phase 1 (schema codification on the optional-then-tighten path) can both begin AND run F2's tighten-to-required step once the backfill is complete.
- **A1 - taxonomy cuts is ratified; the entry-level work batches into Phase 1.** The Fable review (2026-06-09/10) returned "adopt with changes; do not re-cut," and the maintainer worked the Q1-Q12 register on 2026-06-19: the cut is adopted with `relational` renamed `personal` (Q2), the voice axis folded to 5 families (`pastoral` into `care`), the gate neighbor set defined as tree-plus-`confusable_with` (Q4), and the confusable-graph audit (Q6) routed to the existing diff-pair artifacts with no new schema field. The remaining moves (Q3 channel-clone collapse, Q6 audit, Q8 borderline placements, Q9 slug renames) are entry-data work batched into the Phase 1 migration so entries are touched once. **D1 - anchor topics is the only P0 row still open**, deferred pending example review and the expand/randomize-topics question (independent of A1's now-settled structure).
- **Two follow-on blockers surfaced while recording.** (i) C1's quality-MEASUREMENT method is now designed (2026-06-17): the restraint check - genuine register vs caricature, judged against each entry's own failure modes, calibrated on the frozen 60 (see `adherence-gate-spec.md`, "The restraint check (resolving C1)"). It is parked for calibration at gate-build time; the gate still must not ship on distinguishability alone, but the rubric is no longer the open unknown it was. (ii) C2 and F1 are coupled - the maintainer's sustainable audit capacity sizes the program, so F1 (a P1) should be set with C2's operating rule in mind; it is the next decision to make.
