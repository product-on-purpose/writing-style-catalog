...
4. If a conflict is found, classify it: does it involve a Phase-3-recommended (non-fixed) axis, or is it strictly between two Phase-4-fixed values?
   ...
   - **No candidate in the full pool is both compatible and relevant enough, on EVERY recommender-controlled axis involved in the conflict:** only now fall through to the warning path, using the best-scoring (most relevant) candidate from whichever side was originally recommended - the same one that triggered the conflict in the first place. This presents the genuinely best-fitting option with the conflict named, rather than silently swapping in a poor-fitting entry just because it avoids the conflict, and it is reached only after exhausting every axis the skill actually had agency over. This should be rare - it means no stable entry on either conflicting axis is both a real fit and conflict-free - but the design does not assume it cannot happen.
...
2. Attach a conflict warning alongside the composed prompt only when Phase 5 actually reached its warning fallback - both sides fixed, or every recommender-controlled axis involved in the conflict was searched (short list, then widened pool) with no compatible-and-relevant candidate found on any of them. This must match Phase 5's real trigger condition exactly: if Phase 5 resolved the conflict by re-picking any axis, from any search stage, there is nothing to attach - the composed prompt already reflects the clean, resolved set.
