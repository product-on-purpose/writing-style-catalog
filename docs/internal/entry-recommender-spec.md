...
**Status:** draft
**Last updated:** 2026-07-03 by agent (Claude Opus 4.8), revised after a second Codex adversarial review
**Linked plan:** `docs/internal/release-plans/entry-recommender-implementation-plan.md`
**Open questions:** 3 (see Open Questions)
**Revisions:** 2 (see Revisions)
...
- [ ] **AC-4** - Conflicts anywhere in the final four-axis set are resolved by re-picking a recommender-controlled axis when possible, and named when not (both fixed, or no compatible alternative)
- [ ] **AC-5** - Default output is a composed prompt using the resolved set; warn-and-compose is the fallback only when resolution was not possible; `--recommend-only` suppresses composition
...
1. A deterministic pre-filter narrows each axis's full stable candidate list to a short list. The facet fields it scores on are axis-specific, not universal - verified directly against every axis schema [S4]: `domain`/`family` exist only on Format (both required, ADR 0010 enums) [S3]; Voice carries `family` but explicitly no `domain` ("a voice travels across spheres" per its own schema); Tone carries neither and is scored on `spectrum`/`markers` instead; Style carries neither and is scored on `frame`/`classical_mode`/`structural_conventions` instead. Every axis additionally scores on keyword overlap against `when_to_use` and `tells`, the two fields genuinely required on all four axes [S4].
...
This two-stage design keeps the candidate set the model reasons over small (full-corpus reasoning over 97 entries per call is unnecessary and slower than it needs to be) while keeping the final judgment call - which requires real reading comprehension of nuanced `when_to_use`/`tells` prose - with the model rather than a keyword score alone.
...
- **No new dependencies.** The deterministic pre-filter is pure keyword/facet matching over data already in `taxonomy/**/ENTRY.md` frontmatter - no embedding model, no vector store, no new Python package. Consistent with the catalog's "free by default" posture; see Open Questions for whether this proves precise enough at scale.
...
2. **Declined, with reasoning recorded.** The review flagged that this spec and its companion planning docs live under `docs/internal/`, which `AGENTS.md` and the root `CLAUDE.md` mark read-only. This reads correctly from those two files in isolation, but `docs/internal/adr/` and `docs/internal/release-plans/` have an established, repeatedly-exercised exception: they are living documents edited under direct user/maintainer direction, distinct from the genuinely frozen `docs/internal/_working/` and `_LOCAL/`. This exception is not new or invented for this proposal - it is how `docs/internal/agentic-generation-spec.md`, the promotion-and-release runbook, and every `stream-b-*` tracker in this repo were produced and merged. This spec is honestly self-labeled `status: draft - proposed, not yet reviewed by the maintainer` in its own frontmatter, which is the commit rationale the review asked for, and its creation and commit were both direct maintainer instructions in this session, not an agent acting unprompted. No change made.
...
- **[S3]** `docs/internal/adr/0010-domain-and-family-organization.md` and `schemas/format.schema.json` - `domain`/`family` are Format-only facets (both required, ADR 0010 enums); Voice's `schemas/voice.schema.json` carries `family` (a different enum: expert/care/principal/witness/dissident) but explicitly no `domain`. Class A (repo files, verified current 2026-07-02 against an adversarial review finding that the original draft over-generalized this to all four axes).
- **[S4]** `schemas/entry.universal.schema.json` (confirms `when_to_use` and `tells` are required on every axis), `schemas/tone.schema.json` (`spectrum`, `spectrum_position`, `markers` - no `domain`/`family`), `schemas/style.schema.json` (`frame`, `classical_mode`, `structural_conventions` - no `domain`/`family`). Class A (repo files, verified current 2026-07-02).
