...
Each entry follows the atomic-folder pattern: `taxonomy/<axis>/<entry-id>/ENTRY.md` (frontmatter + body), with an `examples/` subdirectory alongside. Every `ENTRY.md` carries an `llm_instruction_phrasing` block, which is what the `compose-instruction` skill assembles into a prompt prefix.
