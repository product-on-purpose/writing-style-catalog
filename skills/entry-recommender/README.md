...
- `scripts/recommend.py --input-file PATH --json` (JSON payload `{"situation": "...", "voice": "...", ...}`, properly JSON-string-escaped, written by a file-write tool - never a shell command - to a scratchpad/temp location, deleted right after) - score the catalog against a situation. This is the path `SKILL.md` requires: situation text is arbitrary, sometimes sensitive user prose, so it must never be interpolated directly into a shell command (shell-injection risk) or hand-substituted into JSON without escaping (malformed-JSON risk, triggered by completely ordinary text like an embedded quote).
- `scripts/recommend.py --stdin --json` - same payload, from stdin. Not recommended for untrusted situation text via a shell heredoc: a heredoc's closing delimiter is plain text matched against the body, so a fixed, predictable delimiter (one documented in this repo's own history, for example) can be deliberately included in the situation text to terminate the heredoc early and reintroduce shell execution.
